...
- 安装页提供版本筛选与创建实例表单；侧边栏的「插件市场」打开独立插件页面。
...
## 插件市场（`pages/plugins.slint`）

- 侧边栏「插件市场」切换至页面 3；有选中实例时刷新兼容列表。
- 页面提供搜索、兼容版本安装、当前实例已安装列表及卸载确认；应用层处理停止实例、事务与错误提示。没有选中实例时，搜索和安装会提示先选择实例。
