...
## 实例列表（`pages/instances.slint`，对应 HMCL `GameListPage`）
...
## 下载与安装（`pages/install.slint`，对应 HMCL `DownloadPage` + `VersionsPage` + `InstallersPage`）
