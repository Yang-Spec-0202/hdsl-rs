...
界面复刻 S0–S7 已落地，以下为首版尚未完成、留待后续开发的部分（与 [UI 页面设计明细](ui-pages.md) 的「待实现切片」一致）：

- **设置持久化**：设置页当前只有界面与本地状态，尚未写入磁盘；重启后不保留。
- **外观生效**：主题模式、主题色、背景图片、字体等仅界面，未实际应用。
- **数据接线**：下载分类仅保留「版本」与「插件」；插件市场列表已接真实数据，下载与插件安装的完整流程、进度与失败提示未接。
- **实例详情**：版本组件的「更换版本」、插件管理的安装/卸载未接真实操作。
- **日志**：运行日志已按级别着色与过滤，但未持久化到文件，未提供导出与「始终置顶」的实际置顶。
- **图形资源**：图标与实例图标为原创占位图，待原创资源替换；候选图标集（Reicon/Tabler）按许可与 AGENTS.md 留在库外。
- **范围外功能**：HMCL 的模组/资源包/世界/原理图管理、账户登录、多人联机、主题包市场、NBT 编辑器不在首版范围。
- **发行验证**：Linux arm64 需 CI 原生运行器验证；真实 Harness 与首批插件的端到端人工检查待做。
